...
parser.add_argument("--prompt_suffix", type=str, default="_boxed")
...
#
if args.temperature < 0.1 and args.temperature > 0:
    exp_dir = os.path.join(args.exp_dir, args.model_name, args.dataset_name, f"temp={args.temperature:.2f}_seed={args.seed}")
if args.n > 64:
    exp_dir = os.path.join(args.exp_dir, args.model_name, args.dataset_name, f"temp={args.temperature:.1f}_seed={args.seed}_n={args.n}")
else:
    exp_dir = os.path.join(args.exp_dir, args.model_name, args.dataset_name, f"temp={args.temperature:.1f}_seed={args.seed}")
#
...
if "Qwen":
    tokenizer = AutoTokenizer.from_pretrained(model_path, cache_dir='/n/netscratch/dam_lab/Lab/sqin/models/qwen')
else:
    tokenizer = AutoTokenizer.from_pretrained(model_path, cache_dir='/n/netscratch/dam_lab/Lab/sqin/models')
